...
def draw_labels_and_boxes(img,boxes,confidences,classids,idxs,colors,labels):
    if (len(idxs)>0): #specifies that there is some object detected
    ...
    for out in outs:
        for detection in out:
            ...
            if (confidence>tconf):
                ...
                centreX,centreY,bwidth,bheight = box.astype('int')

                #derive coordinates of top left corners of bounding boxes
                x = int(centreX-(bwidth/2))
                y = int(centreY-(bheight/2))
